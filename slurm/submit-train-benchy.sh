...
PMIX_MCA_psec=native srun -ul --mpi pmix --network disable_rdzv_get --environment ./env/ngc-brainbert-25.12-alps2.toml \
    ...
    python3 ${ENABLE_DEBUGGING:+-Xfrozen_modules=off} run_train.py hydra.run.dir=${HYDRA_BASE_RUN_DIR:-$PWD/outputs}/$(date +'%Y-%m-%d/%H-%M-%S')-${SLURM_JOB_ID} +exp=spec2vec ++exp.runner.device=cuda ++exp.runner.dist_gpu=True ++task.dist_gpu=True ++exp.runner.num_workers=16 +data=masked_spec +model=masked_tf_model_large +data.data=${PRETRAIN_DATA_DIR}/manifests ++data.format=h5 ++data.val_split=0.01 +task=fixed_mask_pretrain.yaml +criterion=pretrain_masked_criterion +preprocessor=stft ++data.test_split=0.01 ++task.freq_mask_p=0.05 ++task.time_mask_p=0.05 ++exp.runner.total_steps=500000 ++exp.runner.log_step=100000 ++exp.runner.checkpoint_step=100000 ++exp.runner.scheduler.name=reduce_on_plateau
"
