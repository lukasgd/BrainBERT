...
#SBATCH -x nid[006601,006718,007294,007325]
...
PRETRAIN_DATA_DIR=$SCRATCH/BrainBERT/pretrain_data/manifests/
...
srun -ul --environment ./env/ngc-pytorch-25.06.toml bash -c "
    ...
    python3 run_train.py +exp=spec2vec ++exp.runner.device=cuda ++exp.runner.dist_gpu=True ++task.dist_gpu=True ++exp.runner.num_workers=16 +data=masked_spec +model=masked_tf_model_large +data.data=${PRETRAIN_DATA_DIR} ++data.format=h5 ++data.val_split=0.01 +task=fixed_mask_pretrain.yaml +criterion=pretrain_masked_criterion +preprocessor=stft ++data.test_split=0.01 ++task.freq_mask_p=0.05 ++task.time_mask_p=0.05 ++exp.runner.log_step=10 ++exp.runner.total_steps=1000 ++exp.runner.scheduler.name=reduce_on_plateau
"