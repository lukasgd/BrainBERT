...
class FinetuneTask(BaseTask):
    ...
    def get_valid_outs(self, model, valid_loader, criterion, device):
        ...
        if self.cfg.dist_gpu:
            raise NotImplementedError("Distributed ROC AUC not implemented yet")
    ...
    def get_batch_iterator(self, dataset, batch_size, shuffle=True, **kwargs):
        return self.get_data_loader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=finetune_collator, **kwargs)
